...
function Mycards() {
    ...
    const {
        setCurrentPAge,
        cardPerPage,
        currentPage, firstCardIndex, lastCardIndex
    } = usePagination();
            ...
            {cards.length > 0 ? (
                <div className="d-flex w-70 justify-center m-auto gap-5 mb-5 flex-wrap h-50">
                    {currentCards
                        ...
                        .map((card) => (
                            <Card card={card} key={card._id} onDelete={handleDelete} />
                        ))}
                </div>
            ) : <div>
                <p className="fs-4"> There are no cards added by you , click on the add button to start</p>
            </div>}
            {currentCards.length > 0 && <Pagination
